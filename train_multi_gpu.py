...
parser.add_argument('--trainroot', default='./data/train', help='path to dataset')
parser.add_argument('--valroot', default='./data/val', help='path to dataset')
...
def trainIters(model, criterion, plot_every=100):
    ...
    model_optimizer = optim.SGD(cnn.parameters(),lr=base_lr)
    ...
    for epoch in range(1, niter + 1):
        ...
        # if (epoch+model_id) % 1 == 0:
        #     acc = eval_test()
        #     print('the accuracy in %d epoch is %f' % (epoch+model_id, acc))
        # writer.add_scalar('acc', acc, (epoch + model_id - 1))
...
def load_encoderpth(model):
    model_dir = './expr/encoder/'
    if find_new_file(model_dir) is not None:
        model.load_state_dict(torch.load(find_new_file(model_dir)))
        print('load the model %s' % find_new_file(model_dir))
        model_id = re.findall(r'\d+', find_new_file(model_dir))
...
def load_decoderpth(model):
    model_dir = './expr/decoder/'
    if find_new_file(model_dir) is not None:
        model.load_state_dict(torch.load(find_new_file(model_dir)))
        print('load the model %s' % find_new_file(model_dir))
        model_id = re.findall(r'\d+', find_new_file(model_dir))
...
if __name__ == '__main__':
    ...
    # if not os.path.exists('./expr/cnn'):
    #     os.makedirs('./expr/cnn')
    # if not os.path.exists('./expr/encoder'):
    #     os.makedirs('./expr/encoder')
    # if not os.path.exists('./expr/decoder'):
    #     os.makedirs('./expr/decoder')
