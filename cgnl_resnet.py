...
def resnet50(pretrained=False, nl_type=None, nl_nums=None, **kwargs):
    ...
    if pretrained:
        _pretrained = torch.load('/home/kawhi/.torch/models/resnet50-19c8e357.pth')
...
def resnet101(pretrained=False, nl_type=None, nl_nums=None, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 23, 3],
                   nl_type=nl_type, nl_nums=nl_nums, **kwargs)
    if pretrained:
        _pretrained = torch.load('/home/kawhi/.torch/models/resnet101-5d3b4d8f.pth')
...
def resnet152(pretrained=False, nl_type=None, nl_nums=None, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 8, 36, 3],
                   nl_type=nl_type, nl_nums=nl_nums, **kwargs)
    if pretrained:
        _pretrained = torch.load('/home/kawhi/.torch/models/resnet152-b121ed2d.pth')
        _model_dict = load_partial_weight(model, _pretrained, nl_nums, 35)
        model.load_state_dict(_model_dict)
    return model

# model = model_hub(arch='50', pretrained=True, nl_type='cgnl', nl_nums=1, pool_size=4)
cgnl = SpatialCGNLx(256, 256, use_scale=False, groups=8, order=3)
input = Variable(torch.rand(2, 256, 32, 32))
output = cgnl(input)
print(output.size())